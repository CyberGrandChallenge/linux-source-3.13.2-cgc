...
load_cgcos_binary(struct linux_binprm *bprm) {
	...
	current->personality |= PER_CGCOS | ADDR_NO_RANDOMIZE;
	...
	for (i = 0; i < hdr.c_phnum; i++) {
		...
		k = cgc_map(bprm->file, phdr->p_vaddr, phdr, prot, flags, 0);
		if (BAD_ADDR(k)) {
	...
	int prot, int type, unsigned long total_size)
{
	...
	*/
	if (total_size) {
		total_size = CGC_PAGEALIGN(total_size);
...
cgcos_fdwait(int nfds, fd_set __user *readfds, fd_set __user *writefds,
	     struct timeval __user *timeout, int __user *readyfds) {
	int res = sys_select(nfds, readfds, writefds, NULL, timeout);
...
cgcos_allocate(unsigned long len, unsigned long exec, void __user **addr) {
	...
	/* XXX JLW: better constant for this? */
	if (res >= 0xc0000000)
		return (res);
