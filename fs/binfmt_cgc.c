...
static int cgc_parse_args(struct linux_binprm *);
...
static int
cgc_parse_args(struct linux_binprm *bprm) {
	int i, err = 0;
...
cgc_parse_arg(struct linux_binprm *bprm, const char *arg) {
	...
	if (strncmp(arg, arg_name, strlen(arg_name)) == 0) {
		...
		if (hex2bin(seed, arg + strlen(arg_name), seedlen)) {
			ret = -EINVAL;
			goto out_seed;
		}
